...
module.exports.DigimonQualities = {
	...
	"Area Attack - Close Blast": {tags: ["A", "T"], cost: 2, maxRanks: false, desc: "Melee/Ranged: Close Blasts originate in a circular zone adjacent to the user. The circle’s base diameter is 4 Meters. The user may add double their Bit Value to the diameter.", unlocks: [], handler: "attack", attackTag: "[Close Blast]" },

	"Area Attack - Cone": {tags: ["A", "T"], cost: 2, maxRanks: false, desc: "Melee/Ranged: Cone Attacks create a equilateral triangle, or a cone, which originates adjacent to the user. The shape has a base length of 3 meters. The user may add double their Bit Value to the length.", unlocks: [], handler: "attack", attackTag: "[Cone]" },
	...
	"Attack Effect - Fear (N)": {tags: ["A"], cost: 2, maxRanks: 0, desc: "The target takes an Accuracy penalty for attacking the user of this Effect equal to the user’s Bit Value for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Fear]" },
	...
	"Attack Effect - Paralysis (N)": {tags: ["A"], cost: 3, maxRanks: 0, desc: "The target cannot move for the duration of the effect and takes a penalty to their Dodge equal to the user’s Bit value. If the target was Clashing, the Clash ends.", unlocks: [], handler: "attack", attackTag: "[Paralysis]" },

	"Attack Effect - Charm (N)": {tags: ["A"], cost: 3, maxRanks: 0, desc: "The user takes control of the target’s actions for the duration of this Effect. However, for the duration of the Effect, the user loses 2 Simple Actions per round in order to maintain concentration. The Charmed target(s) are still treated as enemies for the purposes of Qualities which affect allies or enemies. If the Charmed target(s) would be hit by the user’s allies, the effect ends. Once an Enemy has been Charmed, it cannot be Charmed again by the same source until combat ends.", unlocks: [], handler: "attack", attackTag: "[Charm]" },
	...
	"Improved Derived Stat - Body": {tags: ["S"], cost: 1, maxRanks: 0, desc: "Whenever you take Improved Derived Stat - Body, increase your Body Derived Stat by 1.", unlocks: [], handler: "addStat", statMods: { Body: 1 }  },

	"Improved Derived Stat - Brains": {tags: ["S"], cost: 1, maxRanks: 0, desc: "Whenever you take Improved Derived Stat - Brains, increase your Brains Derived Stat by 1.", unlocks: [], handler: "addStat", statMods: { Brains: 1 }  },
	...
	"Selective Targeting": {tags: ["S"], cost: 3, maxRanks: false, desc: "The Digimon’s Attacks will no longer be able to Damage Allies or place negative Effects on them such as Poison, Weaken, or Blind. Inversely, enemies the Digimon Attack will not be able to gain benefits from positive Effects such as Healing, Strengthen, or Cleanse. Allies no longer roll Dodge in response to the Digimon’s positive Effect attacks.", unlocks: [], handler: "ability" },
...
module.exports.DigimonQualitiesAdvanced = {
	...
	"Data Specialization - Black Mage": {tags: ["S", "T"], cost: 2, maxRanks: false, desc: "Whenever the Digimon uses a ‘negative’ [Effect] Tagged Attack (an Effect which hinders or damages a target such as Poison, Blind, or Weaken), it gains an additional 2 rounds to the duration after the leftover Accuracy Dice and Resistant are calculated. For reference, this means most negative Effects the Digimon uses will not have a Duration below 3 rounds (5 for Poison).", prereqs: {"Data Optimization - Effect Warrior": 1, "Hybrid Drive - Ranged Striker Adjacent": 1, "Hybrid Drive - Guardian Adjacent": 1}, unlocks: [], handler: "dataSpec", stage: "Ultimate" },

	"Data Specialization - White Mage": {tags: ["S", "T"], cost: 3, maxRanks: false, desc: "Whenever the Digimon uses a ‘positive’ [Effect] Tagged Attack (an Effect which would bolster or heal a target such as Shielding, Cleanse, or Vigor), it adds its Bit Value to the Power of the Effect an additional time.", prereqs: {"Data Optimization - Effect Warrior": 1, "Hybrid Drive - Ranged Striker Adjacent": 1, "Hybrid Drive - Guardian Adjacent": 1}, unlocks: [], handler: "dataSpec", stage: "Ultimate" },
	...
	"Element Master": {tags: ["T"], cost: 2, maxRanks: 0, desc: "Per each Rank in Element Master, the Digimon gains the ability to manipulate an aspect of nature such as earth, fire, or water. The Digimon may only manipulate natural sources of the element: magical, modified, or otherwise GM-decided outliers may not be manipulated. Within its [Burst] radius (consider it Ranged], the Digimon may also change Difficult Terrain into Basic Terrain of its chosen element as a Simple Action, or vise-versa.", prereqs: {"Naturewalk": 1}, unlocks: [], handler: "ability" },

	"Elemental Summoner": {tags: ["S"], cost: 3, maxRanks: false, desc: "Whenever the Digimon’s summoned objects are destroyed, they deal damage to any enemies or allies within a [Burst] radius for damage equal to the Digimon’s Bit Value. The Burst Radius is the same as if the Digimon was using a [Burst] Attack itself. The Digimon’s summoned objects are affected by Qualities such as Selective Targeting, Mobile Artillery, and other effects the Digimon itself has. The summoned objects roll Accuracy for this attack as per normal.", prereqs: {"Summoner": 1}, unlocks: [], handler: "ablity" },
	...
	"Shade Cloak": {tags: ["S"], cost: 2, maxRanks: false, desc: "The Digimon may now apply their Hide in Plain Sight bonus to all allies within its [Burst] radius.", prereqs: {"Hide in Plain Sight": 3}, unlocks: [], handler: "ability" },
