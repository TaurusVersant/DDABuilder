...
module.exports.DigimonSizes = [
	{ id: "Tiny", area: "1x1", squareMeters: "1+", statBonus: {'Body': -2, 'Agility': 2}, notes: "May occupy squares that another Digimon or Tamer are standing in." },
	{ id: "Small", area: "1x1", squareMeters: "1+", statBonus: {'Body': -1, 'Agility': 1}, notes: "May move through squares that other Digimon or Tamers are standing in." },
	{ id: "Medium", area: "1x1", squareMeters: "1+", statBonus: {'Body': 0, 'Agility': 0}, notes: "" },
	{ id: "Large", area: "2x2", squareMeters: "4+", statBonus: {'Body': 1, 'Agility': -1}, notes: "" },
	{ id: "Huge", area: "3x3", squareMeters: "9+", statBonus: {'Body': 2, 'Agility': -1}, notes: "" },
	{ id: "Gigantic", area: "4+x4+", squareMeters: "16+", statBonus: {'Body': 3, 'Agility': -2}, notes: "" }
];
...
module.exports.DigimonQualities = {
	...
	"Area Attack - Cone": {tags: ["A", "T"], cost: 2, maxRanks: false, desc: "Melee/Ranged: Cone Attacks create an equilateral triangle, or a cone, which originates adjacent to the user. The shape has a base length of 3 meters. The user may add their Bit Value to the length.", unlocks: [], handler: "attack", attackTag: "[Cone]" },
	...
	"Attack Effect - Immobilize (N)": {tags: ["A"], cost: 1, maxRanks: false, desc: "The target takes a penalty to their Movement equal to the user’s Bit Value x2 for the duration of the Effect. This Effect may lower a target’s Movement to 0.", unlocks: [], handler: "attack", attackTag: "[Immobilize]" },

	"Attack Effect - Knockback (N)": {tags: ["A"], cost: 1, maxRanks: false, desc: "The target is forcibly pushed away from the user a number of meters equal to the user’s CPU Value. This Effect has no Duration. If the target would be pushed into a solid object such as a wall, use the falling guidelines for the damage it takes. If it would be pushed into a group of enemy Digimon, use the throwing guidelines for the damage each party takes.", unlocks: [], handler: "attack", attackTag: "[Knockback]" },

	"Attack Effect - Taunt (N)": {tags: ["A"], cost: 1, maxRanks: false, desc: "The target takes an Accuracy penalty for attacking anyone who is not the user of the Taunt Effect equal to the user’s CPU Value x2 for the duration of this Effect. If the Attacker uses an Area Attack which has the user as a target, it instead takes a penalty of the user’s CPU-2 for the Accuracy roll.", unlocks: [], handler: "attack", attackTag: "[Taunt]" },

	"Attack Effect - Fear (N)": {tags: ["A"], cost: 1, maxRanks: false, desc: "The target takes an Accuracy penalty for attacking the user of this Effect equal to the user’s Bit Value for the duration of this Effect. If the target was Clashing with the user, the Clash ends, and it may not Initiate Clashes with the user for the duration.", unlocks: [], handler: "attack", attackTag: "[Fear]" },

	"Attack Effect - Poison (N)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target takes a set amount of unalterable Wound Box Damage equal to the user’s Bit Value at the end of each round. Poison’s minimum duration is 3 rounds.", unlocks: [], handler: "attack", attackTag: "[Poison]" },

	"Attack Effect - Confuse (N)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target takes a penalty to Accuracy and Dodge equal to the target’s CPU value or Bit value (whichever is higher), for the duration of the Effect.", unlocks: [], handler: "attack", attackTag: "[Confuse]" },

	"Attack Effect - Stun (N)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target loses a Simple Action each round for the duration of the Effect. If the target was Clashing, the Clash ends.", unlocks: [], handler: "attack", attackTag: "[Stun]" },

	"Attack Effect - Lifesteal (P)": {tags: ["A"], cost: 2, maxRanks: false, desc: "This Attack heals a number of the user’s Wound Boxes equal to the user’s CPU value. If the Attack deals damage less than the user’s CPU value, it instead heals for that amount instead. However, you may use this Attack as a Complex Action in order to double the Lifesteal Effect’s potency. Lifesteal has no Duration.", unlocks: [], handler: "attack", attackTag: "[Lifesteal]" },

	"Attack Effect - Vigor (P)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target gains a bonus to their Dodge score equal to the user’s Bit Value, and to their Movement score equal to the user’s Bit Value doubled, for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Vigor]" },

	"Attack Effect - Fury (P)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target gains a bonus to their Accuracy and Damage scores equal to the user’s Bit value for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Fury]" },

	"Attack Effect - Cleanse (P)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The user may reduce the duration of a number of Effects equal to the leftover Accuracy Dice (for example, if there were 3 Leftover Accuracy dice, it may target up to 3 Effects), and reduce their duration by a number of turns equal to their Bit Value. Even if applied to an [Area] Attack, Cleanse may only target a total number of Effects equal to the highest number of leftover accuracy dice from among the targets.", unlocks: [], handler: "attack", attackTag: "[Cleanse]" },

	"Attack Effect - Haste (P)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target of the Attack gains an additional Simple Action to use for one round. This Effect has no Duration. An Attack with the Haste Effect must be used as a Complex Action.", unlocks: [], handler: "attack", attackTag: "[Haste]" },

	"Attack Effect - Strengthen (P)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target gains a bonus to its Damage and Armor equal to the user’s Bit Value for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Strengthen]" },

	"Attack Effect - Weaken (N)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target takes a penalty to its Damage and Armor equal to the user’s Bit Value for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Weaken]" },

	"Attack Effect - Swiftness (P)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target gains a bonus to its Dodge and Accuracy equal to the user’s Bit Value for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Swiftness]" },

	"Attack Effect - Vigilance (P)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target gains a bonus to its Dodge and Armor equal to the user’s Bit Value for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Vigilance]" },

	"Attack Effect - Distract (N)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target takes a penalty to its Dodge and Accuracy equal to the user’s Bit Value for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Distract]" },

	"Attack Effect - Exploit (N)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target takes a penalty to its Armor and Dodge equal to the user’s Bit Value for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Exploit]" },

	"Attack Effect - Pacify (N)": {tags: ["A"], cost: 2, maxRanks: false, desc: "The target takes a penalty to its Damage and Accuracy equal to the user’s Bit Value for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Pacify]" },

	"Attack Effect - Blind (N)": {tags: ["A"], cost: 3, maxRanks: false, desc: "The target takes a penalty to their Accuracy and Dodge equal to the user’s Bit Value. Additionally, the target automatically fails sight-based Perception checks for the duration of this Effect.", unlocks: [], handler: "attack", attackTag: "[Blind]" },

	"Attack Effect - Paralysis (N)": {tags: ["A"], cost: 3, maxRanks: false, desc: "The target treats all terrain as Difficult Terrain for the duration of the effect and takes a penalty to their Dodge equal to the user’s Bit value. If the target was Clashing, the Clash ends.", unlocks: [], handler: "attack", attackTag: "[Paralysis]" },

	"Attack Effect - Charm (N)": {tags: ["A"], cost: 3, maxRanks: false, desc: "The user takes control of the target’s actions for the duration of this Effect. However, for the duration of the Effect, the user must use 2 Simple Actions per round in order to maintain concentration. The Charmed target(s) are still treated as enemies for the purposes of Qualities which affect allies or enemies. If the Charmed target(s) would be hit by the user’s allies, the effect ends. Once an Enemy has been Charmed, it cannot be Charmed again by the same source until combat ends.", unlocks: [], handler: "attack", attackTag: "[Charm]" },

	"Attack Effect - Shield (P)": {tags: ["A"], cost: 3, maxRanks: false, desc: "The target gains an amount of Temporary Wound Boxes equal to the user’s Bit value. You cannot use a Shield Effect more than once per Round, but may increase the potency to your Bit value x3 by making an Attack with [Shield] a Complex Action instead for that round. Temporary Hit Points are removed at the end of combat.", unlocks: [], handler: "attack", attackTag: "[Shield]" },

	"Attack Effect - Regenerate (P)": {tags: ["A"], cost: 3, maxRanks: false, desc: "The target regains Wound Boxes at the start of each round equal to the user’s Bit Value for the duration of this Effect. While under the effects of Regenerate, the target is also treated as if they had two additional ranks of Resistant.", unlocks: [], handler: "attack", attackTag: "[Regenerate]" },
	...
	"Charge Attack": {tags: ["A"], cost: 1, maxRanks: false, desc: "Choose a [Melee] Attack. By applying the [Charge] Tag to that Attack, the Digimon may use the Attack and move as one Simple Action, instead of needing to use two Simple Actions; one to Attack and one to Move. In short a [Charge] Tag allows the Digimon to use the tagged Attack to move and Attack at the same time.", unlocks: [], handler: "attack", attackTag: "[Charge]" },

	"Combat Awareness I": {tags: ["S"], cost: 1, maxRanks: false, desc: "Add your Ranks in Combat Awareness to your Initiative in the first round of Combat.", unlocks: ["Combat Awareness II"], handler: "ability" },

	"Combat Awareness II": {tags: ["S"], cost: 1, maxRanks: false, desc: "Add your Ranks in Combat Awareness to your Dodge for the first round of Combat.", prereqs: {"Combat Awareness I": 1}, unlocks: ["Combat Awareness III"], handler: "ability" },

	"Combat Awareness III": {tags: ["S"], cost: 1, maxRanks: false, desc: "Add your Ranks in Combat Awareness to your Accuracy for the first round of Combat. The Digimon treats Surprise Rounds triggered against it as a normal round of combat.", prereqs: {"Combat Awareness II": 1}, unlocks: [], handler: "ability" },
	...
	"Data Optimization - Speed Striker": {tags: ["S"], cost: 1, maxRanks: false, desc: "The Digimon gains a +2 bonus to its Base Movement score.", unlocks: ["Data Specialization - Hit and Run", "Data Specialization - Uncatchable Target", "Hybrid Drive - Speed Striker Adjacent"], handler: "dataOpt", statMods: { BaseMovement: 2 } },

	"Data Optimization - Effect Warrior": {tags: ["S"], cost: 2, maxRanks: false, desc: "The Digimon gains a +1 bonus to its base Spec Values, but takes a -2 Armor penalty.", unlocks: ["Data Specialization - Black Mage", "Data Specialization - White Mage", "Hybrid Drive - Effect Warrior Adjacent"], handler: "dataOpt", statMods: { Armor: -2, SpecValues: 1 } },
	...
	"Extra Movement - Digger": {tags: ["S"], cost: 1, maxRanks: false, desc: "The Digimon is capable of burrowing through the ground at a speed equal to its movement, so long as it’s as soft as dirt. Snow or sand are other alternatives.", unlocks: ["Advanced Mobility - Digger"], handler: "addMovement", movementType: "Digging Speed" },

	"Extra Movement - Swimmer": {tags: ["S"], cost: 1, maxRanks: false, desc: "The Digimon is capable of moving through the water at a much faster speed than normal, at a rate equal to its Movement.", unlocks: ["Advanced Mobility - Swimmer"], handler: "addMovement", movementType: "Swim Speed" },
	...
	"Slayer": {tags: ["S"], cost: 1, maxRanks: false, desc: "Slayer is a very specific Quality: when you purchase this Quality, choose a type of target based on a Family or Type of Digimon (Dragon, Demon, Beast, etc). The Digimon gains a bonus equal to its RAM value when making Accuracy checks against targets who match its preferred enemy which was chosen above. Additionally, if the Digimon fits into one of these Families or Types itself, on a Missed Attack it takes unalterable damage equal to the number of stages it is above Fresh. For example, if a Champion stage Digimon misses in this scenario, it would take 3 points of unalterable damage.",  unlocks: [], handler: "ability" },

	"Selective Targeting": {tags: ["S"], cost: 3, maxRanks: false, desc: "The Digimon’s [Area] tagged Attacks will no longer be able to Damage Allies or place negative Effects on them such as Poison, Weaken, or Blind. Inversely, enemies the Digimon attacks will not be able to gain benefits from positive Effects such as Shielding, Strengthen, or Cleanse. This does not apply if the Attack has a single target.", unlocks: [], handler: "ability" },
	...
	"Tumbler": {tags: ["S"], cost: 1, maxRanks: false, desc: "The Digimon gains bonus damage reduction equal to its RAM value doubled when taking damage from falling or being thrown.", unlocks: [], handler: "ability" },

	"Weapon": {tags: ["S, A"], cost: 1, maxRanks: 3, desc: "Your Digimon has a Weapon, special energy, or some type of fighting style it prefers. Attacks which use said Weapon, special energy, or fighting style may gain a [Weapon] Tag. A Digimon may only have a number of [Weapon] Tagged Attacks equal to the Ranks it has in this Quality. Whenever a Digimon uses a [Weapon] Tagged Attack, it gains a bonus to Accuracy and Damage equal to the Ranks in this Quality.", unlocks: ["Chrome Digizoid Weapon", "Black Digizoid Weapon", "Blue Digizoid Weapon", "Gold Digizoid Weapon", "Obsidian Digizoid Weapon", "Red Digizoid Weapon"], handler: "attack", attackTag: "[Weapon]" }
}
...
module.exports.DigimonQualitiesAdvanced = {
	...
	"Data Specialization - Mobile Artillery": {tags: ["S", "T"], cost: 3, maxRanks: false, desc: "The Digimon adds its CPU value to the damage dealt by [Area] tagged Attacks. The Digimon takes a -1 Base Movement Penalty.", prereqs: {"Data Optimization - Ranged Striker": 1, "Hybrid Drive - Speed Striker Adjacent": 1, "Hybrid Drive - Effect Warrior Adjacent": 1}, unlocks: [], handler: "dataSpec", statMods: { BaseMovement: -1 }, stage: "Ultimate" },
	...
	"Element Master": {tags: ["T"], cost: 2, maxRanks: 0, desc: "Per each Rank in Element Master, the Digimon gains the ability to manipulate an aspect of nature such as earth, fire, air, or water. The Digimon may only manipulate natural sources of the element: magical, modified, or otherwise GM-decided outliers may not be manipulated. Within its [Burst][Ranged] radius, the Digimon may also change Difficult Terrain into Basic Terrain of its chosen element as a Simple Action, or vice-versa. They may also change the positioning of the terrain to create structures and trails out of it, moving a number of cubic meters of said Element equal to their Bit value times three.", prereqs: {"Naturewalk": 1}, unlocks: [], handler: "ability" },
	...
	"Overkill": {tags: ["T"], cost: 3, maxRanks: false, desc: "When activating the Huge Power Quality, the Digimon may now also re-roll any dice which show up as 2’s.", prereqs: {"Huge Power": 1}, unlocks: [], handler: "ability" },
